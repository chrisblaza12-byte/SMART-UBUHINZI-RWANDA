          ...
          return <article key={course.title} className={`rounded-[18px] border p-5 shadow-[0_8px_18px_rgba(17,34,27,0.04)] dark:border-[#1d5743] dark:bg-[#123b2f] ${isUnlocked ? "border-[#d7e8e2] bg-white" : "border-[#e2e8e5] bg-[#f5f8f7] opacity-75 dark:bg-[#0b261d]"}`}>
            ...
            <div className="mt-3 space-y-2">{steps.map((step, stepIndex) => {
              ...
              return <div key={step} className="rounded-xl border border-[#e3ece8] bg-[#f8fbfa] p-3 dark:border-[#1d5743] dark:bg-[#0b261d]">
                ...
                {moduleOpen && !moduleComplete && <div className="mt-3 border-t border-[#dce9e4] pt-3 dark:border-[#1d5743]"><div className="space-y-3 text-sm leading-6 text-[#526b65] dark:text-[#c5e2d8]"><p>{course.lesson[stepIndex]}</p><div className="rounded-lg bg-[#eaf8f2] p-3 text-xs dark:bg-[#123b2f]"><p className="font-bold text-[#18794e]">{text.activity}</p><p className="mt-1">{course.activities[stepIndex]}</p></div></div><p className="mt-4 text-sm font-bold text-[#1f4d43] dark:text-[#e5f5ee]">{text.quiz}</p><p className="mt-2 text-sm leading-6 text-[#526b65] dark:text-[#c5e2d8]">{quizQuestion}</p><div className="mt-2 grid gap-2">{quizOptions.map((option, optionIndex) => <label key={option} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-xs ${selectedAnswer === optionIndex ? "border-[#2a7d70] bg-[#eaf8f2]" : "border-[#dce9e4] dark:border-[#1d5743]"}`}><input type="radio" name={`${course.title}-${stepIndex}`} checked={selectedAnswer === optionIndex} onChange={() => setSelectedAnswer(optionIndex)} />{option}</label>)}</div><button type="button" onClick={() => submitQuiz(index, stepIndex)} className="mt-3 rounded-lg bg-[#18794e] px-3 py-2 text-xs font-bold text-white hover:bg-[#12643f]">{text.submit}</button>{quizMessage && <p className="mt-2 text-xs font-semibold text-[#18794e] dark:text-[#86efac]">{quizMessage}</p>}</div>}
              </div>;
            })}</div>
